...
nsWebSocket::EstablishConnection()
{
  ...
  if (mSecure) {
    wsChannel =
      do_CreateInstance("@mozilla.org/network/protocol;1?name=wsspp", &rv);
  } else {
    wsChannel =
      do_CreateInstance("@mozilla.org/network/protocol;1?name=wspp", &rv);
  }
  NS_ENSURE_SUCCESS(rv, rv);
...
nsWebSocket::ParseURL(const nsString& aURL)
{
  ...
  if (scheme.LowerCaseEqualsLiteral("wspp")) {
     mSecure = false;
     mPort = (port == -1) ? DEFAULT_WS_SCHEME_PORT : port;
  } else if (scheme.LowerCaseEqualsLiteral("wsspp")) {
    mSecure = true;
