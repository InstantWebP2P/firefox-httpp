...
static char* nsCRT_strtok(char* string, const char* delims, char* *newStr)
{
...
IsUTF8( const nsACString& aString, bool aRejectNonChar )
  {
    ...
    const char* end = aString.EndReading();
    while ( ptr < end )
      {
        PRUint8 c;
...
NS_IMPL_THREADSAFE_ISUPPORTS11(WebSocketPPChannel,
                               nsIWebSocketChannel,
...
public:
  ...
  CallOnMessageAvailable(WebSocketPPChannel *aChannel,
                         nsCString        &aData,
                         PRInt32           aLen)
    : mChannel(aChannel),
      mData(aData),
...
private:
  ...
  nsRefPtr<WebSocketPPChannel>        mChannel;
  nsCString                         mData;
  PRInt32                           mLen;
};
...
public:
  ...
  CallOnStop(WebSocketPPChannel *aChannel,
             nsresult          aData)
    : mChannel(aChannel),
      mData(aData) {}
...
public:
  ...
  CallOnServerClose(WebSocketPPChannel *aChannel,
                    PRUint16          aCode,
                    nsCString        &aReason)
    : mChannel(aChannel),
      mCode(aCode),
...
public:
  ...
  CallAcknowledge(WebSocketPPChannel *aChannel,
                  PRUint32          aSize)
    : mChannel(aChannel),
      mSize(aSize) {}
  ...
  NS_IMETHOD Run()
  {
    LOG(("WebSocketPPChannel::CallAcknowledge: Size %u\n", mSize));
...
public:
  ...
  OutboundEnqueuer(WebSocketPPChannel *aChannel, OutboundMessage *aMsg)
...
public:
  ...
  bool ConditionallyConnect(nsCString &aStr, WebSocketPPChannel *ws)
  {
    NS_ABORT_IF_FALSE(NS_IsMainThread(), "not main thread");
  ...
  bool Complete(WebSocketPPChannel *aChannel)
  {
    NS_ABORT_IF_FALSE(NS_IsMainThread(), "not main thread");
  ...
  bool Cancel(WebSocketPPChannel *aChannel)
  {
    NS_ABORT_IF_FALSE(NS_IsMainThread(), "not main thread");
    PRInt32 index = IndexOf(aChannel);
  ...
  bool ConnectNext(nsCString &hostName)
  {
    ...
    if (index >= 0) {
      WebSocketPPChannel *chan = mData[index]->mChannel;
...
private:
  ...
  PRInt32 IndexOf(WebSocketPPChannel *aChannel)
  {
    for (PRUint32 i = 0; i < mData.Length(); i++)
...
WebSocketPPChannel::WebSocketPPChannel() :
  mCloseTimeout(20000),
  mOpenTimeout(20000),
  mPingTimeout(0),
  ...
  mDynamicOutput(nsnull)
{
  ...
  LOG(("WebSocketPPChannel::WebSocketPPChannel() %p\n", this));
...
nsresult
WebSocketPPChannel::Create(nsISupports *aOuter, REFNSIID aIID, void **aResult)
{
	nsresult rv;
	BaseWebSocketPPChannel * inst;
...
nsresult
WebSocketPPSSLChannel::Create(nsISupports *aOuter, REFNSIID aIID, void **aResult)
{
	nsresult rv;
	BaseWebSocketPPChannel * inst;
	...
	*aResult = NULL;
	...
	inst = new WebSocketPPSSLChannel;
	NS_ADDREF(inst);
	rv = inst->QueryInterface(aIID, aResult);
	NS_RELEASE(inst);
	return rv;
...
nsresult
WebSocketPPChannel::BeginOpen()
{
  LOG(("WebSocketPPChannel::BeginOpen() %p\n", this));
  NS_ABORT_IF_FALSE(NS_IsMainThread(), "not main thread");

  nsresult rv;

  if (mRedirectCallback) {
    LOG(("WebSocketPPChannel::BeginOpen: Resuming Redirect\n"));
  ...
  if (NS_FAILED(rv)) {
    LOG(("WebSocketPPChannel::BeginOpen: cannot async open\n"));
...
bool
WebSocketPPChannel::IsPersistentFramePtr()
{
...
bool
WebSocketPPChannel::UpdateReadBuffer(PRUint8 *buffer, PRUint32 count,
                                   PRUint32 accumulatedFragments,
                                   PRUint32 *available)
{
  LOG(("WebSocketPPChannel::UpdateReadBuffer() %p [%p %u]\n",
         this, buffer, count));
  ...
  if (mBuffered + count <= mBufferSize) {
    ...
    LOG(("WebSocketPPChannel: update read buffer absorbed %u\n", count));
    ...
    LOG(("WebSocketPPChannel: update read buffer shifted %u\n", mBuffered));
    ...
    LOG(("WebSocketPPChannel: update read buffer extended to %u\n", mBufferSize));
    PRUint8 *old = mBuffer;
...
nsresult
WebSocketPPChannel::ProcessInput(PRUint8 *buffer, PRUint32 count)
{
  LOG(("WebSocketPPChannel::ProcessInput %p [%d %d]\n", this, count, mBuffered));
  ...
  while (avail >= 2) {
    ...
    if (payloadLength < 126) {
      ...
      if (mFramePtr[2] & 0x80) {
        ...
        LOG(("WebSocketPPChannel:: high bit of 64 bit length set"));
    ...
    LOG(("WebSocketPPChannel::ProcessInput: payload %lld avail %lu\n",
         payloadLength, avail));
    ...
    LOG(("WebSocketPPChannel::ProcessInput: Frame accumulated - opcode %d\n",
         opcode));

    if (maskBit) {
      // This is unexpected - the server does not generally send masked
      // frames to the client, but it is allowed
      LOG(("WebSocketPPChannel:: Client RECEIVING masked frame."));
    ...
    if (!finBit && (opcode & kControlFrameMask)) {
      LOG(("WebSocketPPChannel:: fragmented control frame code %d\n", opcode));
      AbortSession(NS_ERROR_ILLEGAL_VALUE);
      return NS_ERROR_ILLEGAL_VALUE;
    }

    if (rsvBits) {
      LOG(("WebSocketPPChannel:: unexpected reserved bits %x\n", rsvBits));
      AbortSession(NS_ERROR_ILLEGAL_VALUE);
      return NS_ERROR_ILLEGAL_VALUE;
    }
    ...
    if (!finBit || opcode == kContinuation) {
      ...
      if ((mFragmentAccumulator != 0) && (opcode != kContinuation)) {
        LOG(("WebSocketPPChannel:: nested fragments\n"));
        AbortSession(NS_ERROR_ILLEGAL_VALUE);
        return NS_ERROR_ILLEGAL_VALUE;
      }

      LOG(("WebSocketPPChannel:: Accumulating Fragment %lld\n", payloadLength));
      ...
      if (finBit) {
        LOG(("WebSocketPPChannel:: Finalizing Fragment\n"));
      ...
      // we have a problem
      LOG(("WebSocketPPChannel:: illegal fragment sequence\n"));
      AbortSession(NS_ERROR_ILLEGAL_VALUE);
      return NS_ERROR_ILLEGAL_VALUE;
    }

    if (mServerClosed) {
      LOG(("WebSocketPPChannel:: ignoring read frame code %d after close\n",
                 opcode));
      // nop
    } else if (mStopped) {
      LOG(("WebSocketPPChannel:: ignoring read frame code %d after completion\n",
           opcode));
    } else if (opcode == kText) {
      LOG(("WebSocketPPChannel:: text frame received\n"));
      if (mListener) {
        ...
        if (!IsUTF8(utf8Data, false)) {
          LOG(("WebSocketPPChannel:: text frame invalid utf-8\n"));
      ...
      if (payloadLength > 125) {
        LOG(("WebSocketPPChannel:: bad control frame code %d length %d\n",
      ...
      if (opcode == kClose) {
        LOG(("WebSocketPPChannel:: close received\n"));
        mServerClosed = 1;
        ...
        if (payloadLength >= 2) {
          ...
          LOG(("WebSocketPPChannel:: close recvd code %u\n", mServerCloseCode));
          ...
          if (msglen > 0) {
            ...
            if (!IsUTF8(mServerCloseReason, false)) {
              LOG(("WebSocketPPChannel:: close frame invalid utf-8\n"));
              AbortSession(NS_ERROR_ILLEGAL_VALUE);
              return NS_ERROR_ILLEGAL_VALUE;
            }
#endif

            LOG(("WebSocketPPChannel:: close msg %s\n",
        ...
        LOG(("WebSocketPPChannel:: ping received\n"));
        ...
        LOG(("WebSocketPPChannel:: pong received\n"));
      } else {
        /* unknown control frame opcode */
        LOG(("WebSocketPPChannel:: unknown control op code %d\n", opcode));
        AbortSession(NS_ERROR_ILLEGAL_VALUE);
        return NS_ERROR_ILLEGAL_VALUE;
      }
      ...
      if (mFragmentAccumulator) {
        ...
        LOG(("WebSocketPPChannel:: Removing Control From Read buffer\n"));
        NS_ABORT_IF_FALSE(mFramePtr + framingLength == payload,
                          "payload offset from frameptr wrong");
      ...
      LOG(("WebSocketPPChannel:: binary frame received\n"));
      if (mListener) {
      ...
      /* unknown opcode */
      LOG(("WebSocketPPChannel:: unknown op code %d\n", opcode));
      AbortSession(NS_ERROR_ILLEGAL_VALUE);
      return NS_ERROR_ILLEGAL_VALUE;
    }
  ...
  if (!IsPersistentFramePtr()) {
    ...
    if (mFragmentAccumulator) {
      LOG(("WebSocketPPChannel:: Setup Buffer due to fragment"));
      ...
      LOG(("WebSocketPPChannel:: Setup Buffer due to partial frame"));
    ...
    LOG(("WebSocketPPChannel:: Internal buffering not needed anymore"));
    mBuffered = 0;
...
void
WebSocketPPChannel::ApplyMask(PRUint32 mask, PRUint8 *data, PRUint64 len)
{
  if (!data || len == 0)
    return;
...
void
WebSocketPPChannel::GeneratePing()
{
...
void
WebSocketPPChannel::GeneratePong(PRUint8 *payload, PRUint32 len)
{
  nsCString *buf = new nsCString();
  buf->SetLength(len);
  if (buf->Length() < len) {
    LOG(("WebSocketPPChannel::GeneratePong Allocation Failure\n"));
    delete buf;
    return;
  }
...
void
WebSocketPPChannel::EnqueueOutgoingMessage(nsDeque &aQueue,
                                         OutboundMessage *aMsg)
{
  NS_ABORT_IF_FALSE(PR_GetCurrentThread() == gSocketThread, "not socket thread");

  LOG(("WebSocketPPChannel::EnqueueOutgoingMessage %p "
...
PRUint16
WebSocketPPChannel::ResultToCloseCode(nsresult resultCode)
{
...
void
WebSocketPPChannel::PrimeNewOutgoingMessage()
{
  LOG(("WebSocketPPChannel::PrimeNewOutgoingMessage() %p\n", this));
  NS_ABORT_IF_FALSE(PR_GetCurrentThread() == gSocketThread, "not socket thread");
  ...
  LOG(("WebSocketPPChannel::PrimeNewOutgoingMessage "
  ...
  *(((PRUint32 *)payload) - 1) = PR_htonl(mask);
  ...
  LOG(("WebSocketPPChannel::PrimeNewOutgoingMessage() using mask %08x\n", mask));
...
void
WebSocketPPChannel::EnsureHdrOut(PRUint32 size)
{
  LOG(("WebSocketPPChannel::EnsureHdrOut() %p [%d]\n", this, size));
...
void
WebSocketPPChannel::CleanupConnection()
{
  LOG(("WebSocketPPChannel::CleanupConnection() %p", this));
...
void
WebSocketPPChannel::StopSession(nsresult reason)
{
  LOG(("WebSocketPPChannel::StopSession() %p [%x]\n", this, reason));
  ...
  if (!mTCPClosed && mTransport && sWebSocketAdmissions &&
    ...
    LOG(("WebSocketPPChannel::StopSession: Wait for Server TCP close"));
...
void
WebSocketPPChannel::AbortSession(nsresult reason)
{
  LOG(("WebSocketPPChannel::AbortSession() %p [reason %x] stopped = %d\n",
  ...
  if (mLingeringCloseTimer) {
    ...
    LOG(("WebSocketPPChannel:: Cleanup connection based on TCP Close"));
    CleanupConnection();
    return;
  }
...
void
WebSocketPPChannel::ReleaseSession()
{
  LOG(("WebSocketPPChannel::ReleaseSession() %p stopped = %d\n",
       this, mStopped));
  NS_ABORT_IF_FALSE(PR_GetCurrentThread() == gSocketThread, "not socket thread");
...
nsresult
WebSocketPPChannel::HandleExtensions()
{
  LOG(("WebSocketPPChannel::HandleExtensions() %p\n", this));
  ...
  if (NS_SUCCEEDED(rv)) {
    if (!extensions.IsEmpty()) {
      if (!extensions.Equals(NS_LITERAL_CSTRING("deflate-stream"))) {
        LOG(("WebSocketPPChannel::OnStartRequest: "
      ...
      if (!mAllowCompression) {
        LOG(("WebSocketPPChannel::HandleExtensions: "
      ...
      if (NS_FAILED(rv)) {
        LOG(("WebSocketPPChannel:: Cannot find compression service\n"));
      ...
      if (NS_FAILED(rv)) {
        LOG(("WebSocketPPChannel:: Cannot find inflate listener\n"));
        AbortSession(NS_ERROR_UNEXPECTED);
        return NS_ERROR_UNEXPECTED;
      }
      ...
      if (NS_FAILED(rv)) {
        LOG(("WebSocketPPChannel:: Cannot find inflate stream\n"));
        AbortSession(NS_ERROR_UNEXPECTED);
        return NS_ERROR_UNEXPECTED;
      }
      ...
      if (!mCompressor->Active()) {
        LOG(("WebSocketPPChannel:: Cannot init deflate object\n"));
        delete mCompressor;
        mCompressor = nsnull;
        AbortSession(NS_ERROR_UNEXPECTED);
        return NS_ERROR_UNEXPECTED;
...
nsresult
WebSocketPPChannel::SetupRequest()
{
  LOG(("WebSocketPPChannel::SetupRequest() %p\n", this));
  ...
  mHttpChannel->SetRequestHeader(NS_LITERAL_CSTRING("Sec-WebSocket-Key"),
                                 secKeyString, false);
  LOG(("WebSocketPPChannel::SetupRequest: client key %s\n", secKeyString.get()));
  ...
  NS_ENSURE_SUCCESS(rv, rv);
  LOG(("WebSocketPPChannel::SetupRequest: expected server key %s\n",
       mHashedSecret.get()));
...
nsresult
WebSocketPPChannel::ApplyForAdmission()
{
  LOG(("WebSocketPPChannel::ApplyForAdmission() %p\n", this));
  ...
  LOG(("WebSocketPPChannel::ApplyForAdmission: checking for concurrent open\n"));
  nsCOMPtr<nsIThread> mainThread;
  NS_GetMainThread(getter_AddRefs(mainThread));
...
nsresult
WebSocketPPChannel::StartWebsocketData()
{
  LOG(("WebSocketPPChannel::StartWebsocketData() %p", this));

  if (sWebSocketAdmissions &&
    sWebSocketAdmissions->ConnectedCount() > mMaxConcurrentConnections) {
    LOG(("WebSocketPPChannel max concurrency %d exceeded "
...
NS_IMETHODIMP
WebSocketPPChannel::OnLookupComplete(nsICancelable *aRequest,
                                     nsIDNSRecord *aRecord,
                                     nsresult aStatus)
{
  LOG(("WebSocketPPChannel::OnLookupComplete() %p [%p %p %x]\n",
  ...
  if (NS_FAILED(aStatus)) {
    LOG(("WebSocketPPChannel::OnLookupComplete: No DNS Response\n"));
  } else {
    nsresult rv = aRecord->GetNextAddrAsString(mAddress);
    if (NS_FAILED(rv))
      LOG(("WebSocketPPChannel::OnLookupComplete: Failed GetNextAddr\n"));
  }

  if (sWebSocketAdmissions->ConditionallyConnect(mAddress, this)) {
    LOG(("WebSocketPPChannel::OnLookupComplete: Proceeding with Open\n"));
  } else {
    LOG(("WebSocketPPChannel::OnLookupComplete: Deferring Open\n"));
  }
...
NS_IMETHODIMP
WebSocketPPChannel::GetInterface(const nsIID & iid, void **result NS_OUTPARAM)
{
  LOG(("WebSocketPPChannel::GetInterface() %p\n", this));
...
NS_IMETHODIMP
WebSocketPPChannel::AsyncOnChannelRedirect(
                    nsIChannel *oldChannel,
                    nsIChannel *newChannel,
                    PRUint32 flags,
                    nsIAsyncVerifyRedirectCallback *callback)
{
  LOG(("WebSocketPPChannel::AsyncOnChannelRedirect() %p\n", this));
  nsresult rv;
  ...
  if (!mAutoFollowRedirects) {
    ...
    rv = clonedNewURI->SetScheme(NS_LITERAL_CSTRING("wspp"));
    NS_ENSURE_SUCCESS(rv, rv);
    ...
    // currentURI is expected to be wspp or wsspp
    bool currentIsHttps = false;
    rv = currentURI->SchemeIs("wsspp", &currentIsHttps);
    NS_ENSURE_SUCCESS(rv, rv);
    ...
    if (!(!currentIsHttps && newuriIsHttps && uriEqual)) {
      ...
      LOG(("WebSocketPPChannel: Redirect to %s denied by configuration\n",
           newSpec.get()));
      return NS_ERROR_FAILURE;
    }
  }
  ...
  if (mEncrypted && !newuriIsHttps) {
    ...
    if (NS_SUCCEEDED(newuri->GetSpec(spec)))
      LOG(("WebSocketPPChannel: Redirect to %s violates encryption rule\n",
           spec.get()));
    return NS_ERROR_FAILURE;
  }
  ...
  if (NS_FAILED(rv)) {
    LOG(("WebSocketPPChannel: Redirect could not QI to HTTP\n"));
    return rv;
  }
  ...
  if (NS_FAILED(rv)) {
    LOG(("WebSocketPPChannel: Redirect could not QI to HTTP Upgrade\n"));
    return rv;
  }
  ...
  if (mEncrypted)
    rv = mURI->SetScheme(NS_LITERAL_CSTRING("wsspp"));
  else
    rv = mURI->SetScheme(NS_LITERAL_CSTRING("wspp"));

  mHttpChannel = newHttpChannel;
  mChannel = newUpgradeChannel;
  rv = SetupRequest();
  if (NS_FAILED(rv)) {
    LOG(("WebSocketPPChannel: Redirect could not SetupRequest()\n"));
    return rv;
  }
  ...
  if (NS_FAILED(rv)) {
    LOG(("WebSocketPPChannel: Redirect failed due to DNS failure\n"));
    mRedirectCallback = nsnull;
    return rv;
  }
...
NS_IMETHODIMP
WebSocketPPChannel::Notify(nsITimer *timer)
{
  LOG(("WebSocketPPChannel::Notify() %p [%p]\n", this, timer));
  ...
  if (timer == mCloseTimer) {
    ...
    LOG(("WebSocketPPChannel:: Expecting Server Close - Timed Out\n"));
    ...
    mOpenTimer = nsnull;
    LOG(("WebSocketPPChannel:: Connection Timed Out\n"));
    if (mStopped || mServerClosed)                /* no longer relevant */
      return NS_OK;
    ...
    if (!mPingOutstanding) {
      LOG(("nsWebSocketPPChannel:: Generating Ping\n"));
      mPingOutstanding = 1;
      GeneratePing();
      mPingTimer->InitWithCallback(this, mPingResponseTimeout,
                                   nsITimer::TYPE_ONE_SHOT);
    } else {
      LOG(("nsWebSocketPPChannel:: Timed out Ping\n"));
      mPingTimer = nsnull;
      AbortSession(NS_ERROR_NET_TIMEOUT);
    }
  } else if (timer == mLingeringCloseTimer) {
    LOG(("WebSocketPPChannel:: Lingering Close Timer"));
    CleanupConnection();
  } else {
...
NS_IMETHODIMP
WebSocketPPChannel::GetSecurityInfo(nsISupports **aSecurityInfo)
{
  LOG(("WebSocketPPChannel::GetSecurityInfo() %p\n", this));
  NS_ABORT_IF_FALSE(NS_IsMainThread(), "not main thread");
...
NS_IMETHODIMP
WebSocketPPChannel::AsyncOpen(nsIURI *aURI,
                            const nsACString &aOrigin,
                            nsIWebSocketListener *aListener,
                            nsISupports *aContext)
{
  LOG(("WebSocketPPChannel::AsyncOpen() %p\n", this));

  NS_ABORT_IF_FALSE(NS_IsMainThread(), "not main thread");

  if (!aURI || !aListener) {
    LOG(("WebSocketPPChannel::AsyncOpen() Uri or Listener null"));
    return NS_ERROR_UNEXPECTED;
  }
  ...
  if (mPingTimeout) {
    ...
    if (NS_FAILED(rv)) {
      ...
      LOG(("WebSocketPPChannel will generate ping after %d ms of receive silence\n",
           mPingTimeout));
  ...
  if (mEncrypted)
    rv = localURI->SetScheme(NS_LITERAL_CSTRING("httpps"));
  else
    rv = localURI->SetScheme(NS_LITERAL_CSTRING("httpp"));
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  if (NS_FAILED(rv)) {
    NS_WARNING("WebSocketPPChannel: unable to continue without ioservice2");
    return rv;
  }
...
NS_IMETHODIMP
WebSocketPPChannel::Close(PRUint16 code, const nsACString & reason)
{
  LOG(("WebSocketPPChannel::Close() %p\n", this));
  NS_ABORT_IF_FALSE(NS_IsMainThread(), "not main thread");

  if (!mTransport) {
    LOG(("WebSocketPPChannel::Close() without transport - aborting."));
    AbortSession(NS_ERROR_NOT_CONNECTED);
    return NS_ERROR_NOT_CONNECTED;
  }

  if (mRequestedClose) {
    LOG(("WebSocketPPChannel:: Double close error\n"));
    return NS_ERROR_UNEXPECTED;
  }
...
NS_IMETHODIMP
WebSocketPPChannel::SendMsg(const nsACString &aMsg)
{
  LOG(("WebSocketPPChannel::SendMsg() %p\n", this));

  return SendMsgCommon(&aMsg, false, aMsg.Length());
}

NS_IMETHODIMP
WebSocketPPChannel::SendBinaryMsg(const nsACString &aMsg)
{
  LOG(("WebSocketPPChannel::SendBinaryMsg() %p len=%d\n", this, aMsg.Length()));
  return SendMsgCommon(&aMsg, true, aMsg.Length());
}

NS_IMETHODIMP
WebSocketPPChannel::SendBinaryStream(nsIInputStream *aStream, PRUint32 aLength)
{
  LOG(("WebSocketPPChannel::SendBinaryStream() %p\n", this));

  return SendMsgCommon(nsnull, true, aLength, aStream);
}

nsresult
WebSocketPPChannel::SendMsgCommon(const nsACString *aMsg, bool aIsBinary,
                                PRUint32 aLength, nsIInputStream *aStream)
{
  NS_ABORT_IF_FALSE(NS_IsMainThread(), "not main thread");

  if (mRequestedClose) {
    LOG(("WebSocketPPChannel:: Error: send when closed\n"));
    return NS_ERROR_UNEXPECTED;
  }

  if (mStopped) {
    LOG(("WebSocketPPChannel:: Error: send when stopped\n"));
    return NS_ERROR_NOT_CONNECTED;
  }

  if (aLength > mMaxMessageSize) {
    LOG(("WebSocketPPChannel:: Error: message too big\n"));
    return NS_ERROR_FILE_TOO_BIG;
  }
...
NS_IMETHODIMP
WebSocketPPChannel::OnTransportAvailable(nsISocketTransport *aTransport,
                                       nsIAsyncInputStream *aSocketIn,
                                       nsIAsyncOutputStream *aSocketOut)
{
  LOG(("WebSocketPPChannel::OnTransportAvailable %p [%p %p %p] rcvdonstart=%d\n",
...
NS_IMETHODIMP
WebSocketPPChannel::OnStartRequest(nsIRequest *aRequest,
                                 nsISupports *aContext)
{
  LOG(("WebSocketPPChannel::OnStartRequest(): %p [%p %p] recvdhttpupgrade=%d\n",
  ...
  if (sWebSocketAdmissions->Complete(this))
    LOG(("WebSocketPPChannel::OnStartRequest: Starting Pending Open\n"));
  else
    LOG(("WebSocketPPChannel::OnStartRequest: No More Pending Opens\n"));
  ...
  if (mStopped) {
    LOG(("WebSocketPPChannel::OnStartRequest: Channel Already Done\n"));
    AbortSession(NS_ERROR_CONNECTION_REFUSED);
  ...
  if (NS_FAILED(rv)) {
    LOG(("WebSocketPPChannel::OnStartRequest: No HTTP Response\n"));
    AbortSession(NS_ERROR_CONNECTION_REFUSED);
    return NS_ERROR_CONNECTION_REFUSED;
  }

  LOG(("WebSocketPPChannel::OnStartRequest: HTTP status %d\n", status));
  if (status != 101) {
    AbortSession(NS_ERROR_CONNECTION_REFUSED);
    return NS_ERROR_CONNECTION_REFUSED;
  }
  ...
  if (NS_SUCCEEDED(rv)) {
    ...
    if (!respUpgrade.IsEmpty()) {
      ...
      ///while ((token = nsCRT_strtok(val, ", \t", &val))) {
	  while ((token = nsCRT_strtok(val, ", \t", &val))) {
  ...
  if (NS_FAILED(rv) ||
    ...
    LOG(("WebSocketPPChannel::OnStartRequest: Expected %s recevied %s\n",
  ...
  LOG(("WebSocketPPChannel::OnStartRequest: Notifying Listener %p\n",
       mListener.get()));
...
NS_IMETHODIMP
WebSocketPPChannel::OnStopRequest(nsIRequest *aRequest,
                                  nsISupports *aContext,
                                  nsresult aStatusCode)
{
  LOG(("WebSocketPPChannel::OnStopRequest() %p [%p %p %x]\n",
...
NS_IMETHODIMP
WebSocketPPChannel::OnInputStreamReady(nsIAsyncInputStream *aStream)
{
  LOG(("WebSocketPPChannel::OnInputStreamReady() %p\n", this));
  NS_ABORT_IF_FALSE(PR_GetCurrentThread() == gSocketThread, "not socket thread");
  ...
  do {
    ...
    LOG(("WebSocketPPChannel::OnInputStreamReady: read %u rv %x\n", count, rv));
...
NS_IMETHODIMP
WebSocketPPChannel::OnOutputStreamReady(nsIAsyncOutputStream *aStream)
{
  LOG(("WebSocketPPChannel::OnOutputStreamReady() %p\n", this));
  NS_ABORT_IF_FALSE(PR_GetCurrentThread() == gSocketThread, "not socket thread");
  nsresult rv;
  ...
  while (mCurrentOut && mSocketOut) {
    ...
    if (mHdrOut) {
      ...
      LOG(("WebSocketPPChannel::OnOutputStreamReady: "
    ...
    if (toSend == 0) {
      ...
      LOG(("WebSocketPPChannel::OnOutputStreamReady: write %u rv %x\n",
           amtSent, rv));
...
NS_IMETHODIMP
WebSocketPPChannel::OnDataAvailable(nsIRequest *aRequest,
                                    nsISupports *aContext,
                                    nsIInputStream *aInputStream,
                                    PRUint32 aOffset,
                                    PRUint32 aCount)
{
  LOG(("WebSocketPPChannel::OnDataAvailable() %p [%p %p %p %u %u]\n",
         this, aRequest, aContext, aInputStream, aOffset, aCount));

  if (aContext == mSocketIn) {
    // This is the deflate decoder

    LOG(("WebSocketPPChannel::OnDataAvailable: Deflate Data %u\n",
             aCount));
    ...
    while (aCount > 0) {
      ...
      LOG(("WebSocketPPChannel::OnDataAvailable: InflateRead read %u rv %x\n",
           count, rv));
  ...
  if (aContext == mSocketOut) {
    ...
    while (aCount > 0) {
      ...
      LOG(("WebSocketPPChannel::OnDataAvailable: DeflateWrite read %u rv %x\n",
           count, rv));
      if (NS_FAILED(rv) || count == 0) {
        AbortSession(rv);
        break;
  ...
  LOG(("WebSocketPPChannel::OnDataAvailable: HTTP data unexpected len>=%u\n",
         aCount));
